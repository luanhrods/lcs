  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#1E1E1E] text-white font-sans relative overflow-hidden animate-fade-in">
        <header className="container mx-auto px-4 sm:px-6 py-6 flex justify-between items-center relative z-10">
          <div className="flex items-center space-x-2">
            <Image src="/images/logo.png" alt="Enchantè Digital Logo" width={120} height={38} />
          </div>
          <nav className="hidden md:flex space-x-8">
            <button onClick={() => scrollToSection(servicesRef)} className="text-gray-300 hover:text-white text-sm md:text-base font-inter transition-colors duration-300">Serviços</button>
            <button onClick={() => scrollToSection(testimonialsRef)} className="text-gray-300 hover:text-white text-sm md:text-base font-inter transition-colors duration-300">Referências</button>
            <button onClick={() => scrollToSection(contactRef)} className="text-gray-300 hover:text-white text-sm md:text-base font-inter transition-colors duration-300">Contato</button>
          </nav>
          <Link href="https://tally.so/r/3qKJl7" target="_blank" rel="noopener noreferrer" className="hidden md:block bg-white text-[#1E1E1E] px-4 py-2 rounded-md text-sm md:text-base font-medium font-inter transition-all duration-300 hover:bg-gray-200 hover:scale-105">
            Fale conosco agora
          </Link>
        ...
        <div className={`fixed top-0 right-0 bottom-0 w-64 bg-[#2A2A2A] z-50 transform transition-transform duration-300 ease-in-out ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="p-6">
            ...
            <div className="mt-8 flex flex-col space-y-4">
              <button onClick={() => scrollToSection(servicesRef)} className="block w-full text-left py-2 text-gray-300 hover:text-white text-base font-inter transition-colors duration-300">Serviços</button>
              <button onClick={() => scrollToSection(testimonialsRef)} className="block w-full text-left py-2 text-gray-300 hover:text-white text-base font-inter transition-colors duration-300">Referências</button>
              <button onClick={() => scrollToSection(contactRef)} className="block w-full text-left py-2 text-gray-300 hover:text-white text-base font-inter transition-colors duration-300">Contato</button>
              <Link href="https://tally.so/r/3qKJl7" target="_blank" rel="noopener noreferrer" className="mt-4 w-full bg-white text-[#1E1E1E] px-4 py-2 rounded-md text-base font-medium font-inter hover:bg-gray-200 transition-colors duration-300 text-center">
                Fale conosco agora
              </Link>
            </div>
          </div>
        ...
        <main className="pb-20 relative z-10">
          <section className="container mx-auto px-4 sm:px-6 pt-20 pb-16">
            <div className="flex flex-col md:flex-row items-start justify-between">
              <div className="md:w-1/2 mb-16 md:mb-0">
                <h1 className="text-4xl sm:text-5xl md:text-6xl font-semibold mb-8 leading-tight font-montserrat">
                ...
                <Link href="https://tally.so/r/nWpy7N" 
                  ...
                  className="bg-white text-[#1E1E1E] px-6 py-3 rounded-md text-base font-medium inline-flex items-center font-inter hover:bg-gray-200 transition-all duration-300 md:px-4 md:py-2 md:text-sm transform hover:scale-105">
                  Saiba mais
                  ...
                  <div key={index} className="bg-[#2A2A2A] p-6 rounded-lg shadow-lg">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center">
                        ...
                        <h3 className="text-xl font-semibold font-montserrat text-white">{item.title}</h3>
                      </div>
                      <button 
                    ...
                    <div 
                      className={`overflow-hidden transition-all duration-300 ease-in-out ${
                        expandedCards[index] ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'
                      }`}
                    >
          ...
          <section className="container mx-auto px-6 py-24">
            <h2 className="text-3xl font-semibold mb-16 flex items-center justify-center font-montserrat text-center">
              O que oferecemos?
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-12">
              {[
                ...
                <Link href={service.link} key={index} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center text-center p-6 rounded-lg transition-all duration-300 ease-in-out hover:bg-[#2A2A2A] active:scale-95 transform hover:scale-105">
                  ...
                  <h3 className="text-lg font-semibold mb-2 font-montserrat">{service.title}</h3>
                  <p className="text-gray-300 text-sm font-inter h-20 flex items-center justify-center">{service.description}</p>
                </Link>
              ))}
            </div>
          </section>
              ...
              {[
                ...
                <div key={index} className={`bg-[#2A2A2A] p-8 rounded-lg ${item.offset} transition-all duration-300 ease-in-out transform hover:scale-105`}>
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <h4 className="font-semibold text-base font-montserrat text-white">{item.name}</h4>
                      <p className="text-sm text-gray-400 font-inter">{item.position}</p>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <section className="bg-[#2A2A2A] text-white py-24" ref={contactRef}>
            <div className="container mx-auto px-6">
              <div className="flex flex-col md:flex-row items-center">
                <h2 className="text-3xl md:text-4xl font-semibold mb-8 md:mb-0 font-montserrat md:w-1/2 text-center md:text-left">Transforme seus sonhos<br />em realidade</h2>
                <Link href="https://wa.me/5511962140166?text=Ea%C3%AD%20gente,%20tudo%20bem?%20Eu%20gostaria%20de%20saber%20mais%20sobre%20todos%20os%20servi%C3%A7os%20que%20voc%C3%AAs%20tem,%20por%20favor."
                  target="_blank" rel="noopener noreferrer"
                  className="bg-white text-[#1E1E1E] px-8 py-3 rounded-md text-sm md:text-base font-medium inline-flex items-center font-inter hover:bg-gray-200 transition-all duration-300 transform hover:scale-105">
                  Nosso WhatsApp
          ...
          <div className="container mx-auto px-6">
            <div className="flex flex-col md:flex-row justify-between items-center">
              <div className="flex items-center space-x-2 mb-4 md:mb-0">
                <Image src="/images/logo.png" alt="Enchantè Digital Logo" width={80} height={28} />
              </div>
              <nav className="flex flex-wrap justify-center space-x-4 md:space-x-6 mb-4 md:mb-0">
                <Link href="https://tally.so/r/3qKJl7" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white text-sm font-inter transition-colors duration-300">Ajuda</Link>
                <Link href="https://tally.so/r/w4N4Qk" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white text-sm font-inter transition-colors duration-300">Política de Privacidade</Link>
                <Link href="https://tally.so/r/w4N4Qk" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white text-sm font-inter transition-colors duration-300">Política de Dados</Link>
                <Link href="https://linkedin.com/company/enchantedigital/" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white text-sm font-inter transition-colors duration-300">
